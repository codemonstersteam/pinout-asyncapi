...
change-delta could not make (`change-delta.md` §5.1): does a factory/product-method pair occupy
**one** row in `module-tree.md` §3 or **two**? The tree already ships two such pairs
(`BuildSpecLoader`/`FileSpecLoader.Load`, `BuildReportWriter`/`ReportWriter.Write`) and splits both,
each factory carrying its own `contracts.md` §4 unit-test row. The BRD (N7/D1) assumed the opposite —
a rename in place, one node each, §4's total pinned at 49.
...
renumbering map in `module-tree.md` §3.
