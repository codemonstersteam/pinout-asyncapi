# slice-01-validate — module contracts, message catalog, tests, component scenarios (change 001-pipe-arity-coverage-gaps)

> **Change-scoped evolution** of [`../../contracts.md`](../../contracts.md), lane `patch`. Companion to
> [`module-tree.md`](./module-tree.md) in this same folder (Steps 4/5/6/8 of `program-design`). Frozen
> contracts referenced throughout — **untouched by this change** (BRD N1):
...
>
> **What changed vs the canonical document:** §1 (+`ComparisonInput`, `ContractParser`, `Reporter`;
> + an explicit **Owner** column), §3 (`NewConsumedContract` → `BuildContractParser` +
> `ContractParser.Parse`; `FoldReport` → `BuildReporter` + `Reporter.Fold`; `NewComparison`'s argument
> becomes the named join; `cli.ResolveExitCode` documents totality per D3; `ProcessValidate` describes
> bind-then-chain), §4 (two renames + two new factory rows; total **49 → 51** — see the note there,
> this contradicts BRD N7 and the BRD is wrong), §5 (+3 Then-step rows + the C2 grid matrix),
> §6 (formula and gate restated, scenario **9** added). §2 is unchanged and reproduced verbatim.
...
| **`ContractParser`** | **collaborator** | **`BuildContractParser`** | **NEW (shape).** Unexported field: the expected consumer name (an already-`NewConfig`-validated config scalar). Performs no I/O. Exists so `Parse` takes one data entity. |
| `ConsumedContract` | `Entity` | **`ContractParser.Parse`** (was `NewConsumedContract`) | valid-by-construction; built only via `Parse`. |
...
| **`Reporter`** | **collaborator** | **`BuildReporter`** | **NEW (shape).** Unexported field: the `clock.Clock` port. Performs no I/O. Exists so `Fold` takes one data entity and so the slice's only clock read is enclosed (D10). |
| `Report` | DTO | **`Reporter.Fold`** (was `FoldReport`) | canon-`1.1` shape, `report.schema.json`. |
...
**Ownership edges this change creates** (for the ticketer): `ContractParser.Parse` depends on
`BuildContractParser`; `Reporter.Fold` depends on `BuildReporter`; `NewComparison` depends on the
owners of all three `ComparisonInput` fields (`NewConfig`, `ContractParser.Parse`,
`DeriveProviderChannels`); `ProcessValidate` (rewiring) depends on all four factories.

## 2. Error model

Unchanged by this change; the only edit is the two renames in the "Raised by" column
(`NewConsumedContract` → `ContractParser.Parse`). `ErrParseError` → `PARSE_ERROR` → exit 3 holds
verbatim (BRD, `errors.go:21`).
...
(D3/C4, see the two cards below).

## 3. Module contracts

Only behavioral modules get a card; `domain.go`/`errors.go` (types/sentinels) and `register.go`
(wiring) do not (`program-design` Step 5). Cards **changed by this change** are marked **(changed)**;
all others are reproduced unchanged.
...
- **Note (defense-in-depth, C1):** the "argc ≠ 1 never reaches the pipe" invariant is held by **two** independent guards — `cobra.ExactArgs(1)` and `Parse`'s own guard on `args[0]`. Neither is removed. It is proven through the binary (`cmd/app/main_test.go`, 0-arg and 2-arg cases), **not** by a unit test of the adapter (C0 stands: no `adapter_test.go`).
- **Note (C0 scope — which `Parse` the rule is about):** C0 forbids unit-testing the **ingress adapter**, i.e. the package-level `cli.Parse` / `cli.ResolveExitCode` (in Go: `validate.Parse(` / `validate.ResolveExitCode(` of `internal/validate/adapter.go`, called **unqualified**). It does **not** reach `ContractParser.Parse(RawContract)` — the domain method introduced by this change (§`ContractParser.Parse`, ADR-001, ticket-04), which is core logic and **is** directly unit-tested by §4 rows 7–8. Mechanical form: `grep -nE '(^|[^.[:alnum:]_])(Parse|ResolveExitCode)\(' internal/validate/*_test.go` must be empty; a receiver-qualified `.Parse(` is a different symbol and is deliberately excluded.

### `ProcessValidate` *(changed — rewiring only)*

- **Signature:** `ProcessValidate(inv: Invocation, deps: Deps) -> Result<Report, Error>` — **unchanged** (BRD N4; the I1c golden depends on it being callable identically before and after)
- **Input (data):** `Invocation`
- **Dependencies:** `Deps{Clock clock.Clock, HTTPClient *http.Client}` — **unchanged**
- **io:** `none`
...
- **Consequent:** success → `Report`; failure → the first short-circuiting step's `Error`, untransformed. **The set of reachable errors, their order, the exit-code grid and the report bytes are identical to the pre-change pipe** — the binds are total and perform no I/O.
- **Not unit-tested** (head, Step 8.1). The `internal/validate` determinism test that drives it (BRD I1c, `testdata/report.golden.json`) is a **D10 determinism regression anchor, not a head unit test** — the same documented, non-formula status `internal/validate/report_json_shape_test.go` already carries. It adds **no** row to §4.

### `cli.ResolveExitCode` *(changed — documentation only, D3)*
...
- **Antecedent:** **none — the function is total over ANY `error`**, not merely over the closed sentinel taxonomy. The `default:` arm is the deliberate out-of-taxonomy fallback and it returns **3**.
  - **Concrete reachable instance:** a `ReportWriter.Write` disk/marshal failure (`io_report.go`) wraps no sentinel; it lands on `default:` → exit **3**. This is pinned by one case in `cmd/app/main_test.go` (BRD C4), **not** by a component scenario — a report-write failure is not one of the 7 adapter branches and a 10th scenario would break §6's gate.
  - **Deliberately unasserted (debt, not fixed here):** on that path the synthesized stdout body carries `errors[0].code == ""` and no `subject`, which is **schema-invalid** against `report.schema.json`. Pinning or fixing it is a behaviour change outside the `patch` envelope (BRD N1); recorded as a follow-up debt item requiring a frozen-contract decision.
- **Consequent:** exactly one of `{0,1,2,3}`, for every possible input.
- **Not unit-tested** (adapter, Step 8.1 — C0/Q2 stand verbatim: no `adapter_test.go`, no §4 row). The full 7-row grid is proven **through the binary** instead; the matrix is in §5.
...
### `BuildContractParser` *(NEW — factory half of the reshaped node 7)*
...
- **Consequent:** a `ContractParser` carrying that name. **No failure branch** (construction only) — this is why hoisting it ahead of the chain is behaviour-preserving.

### `ContractParser.Parse` *(changed — was `NewConsumedContract(raw, expectedConsumer)`)*

- **Signature:** `(p ContractParser) Parse(raw: RawContract) -> Result<ConsumedContract, Error>`
- **Input (data):** `RawContract` — **one** data entity (the second argument moved to construction)
...
- **Antecedent (unchanged, verbatim):** decodes as valid YAML/JSON; `schema_version == "1.0"`; `consumer == p.expectedConsumer`; `provenance.captured_hash` matches `^sha256:[0-9a-f]{64}$`; `channels` non-empty; each channel has non-empty `address`/`protocol` and at least one of `sends`/`receives`.
- **Consequent (unchanged, verbatim):** success → `ConsumedContract`; failure (any antecedent clause) → `ErrContractInvalid` → `PARSE_ERROR` → exit 3.
- **Subtype, not guard:** still the only way a `ConsumedContract` comes to exist.
...
### `NewComparison` *(changed — argument becomes the named join `ComparisonInput`)*
...
- **Antecedent (unchanged, verbatim):** every address in `in.Config.Consumer.Channels` is present among `in.Contract.Channels[].Address`.
- **Consequent (unchanged, verbatim):** success → `Comparison`; failure → `ErrConfigInvalid` (`CONFIG_ERROR`) → exit 2 — still before any report is written.
- **Fixture trap this card exists to prevent (BRD C3):** the `incompatible/` fixture's extra channel address MUST be present in the consumed-contract too. An address in the config but absent from the contract trips **this** antecedent → exit **2**, not the exit **1** verdict scenario 9 is meant to prove.
...
### `BuildReporter` *(NEW — factory half of the reshaped node 17)*
...
- **D10 tightened:** `grep -rn 'time.Now()' internal/validate/` must still match only `register.go`; a counting fake must observe exactly **1** `Now()` call across one `Fold` (BRD I1a/I1b).

### `Reporter.Fold` *(changed — was `FoldReport(outcome)` + `Dependencies: clock.Clock`)*
...
- **io:** `none`
- **What it does (unchanged):** assembles the canon-`1.1` `Report`: `compatible ⇔ errors == []`, `errors[]` (from `Violations`), `provenance` (echoed), `uncovered_channels[]`, `generated_at` (from the bound clock), `validator`/`interaction` build-time constants.
- **Antecedent:** a valid `Outcome`.
- **Consequent:** `Report`, total — no failure branch. **Byte-identical** to the pre-change `FoldReport` for the same `Outcome` and the same instant (proven by the I2 baseline + the I1c golden).
...
### `ReportWriter.Write` *(changed — documentation only; no code change)*
...
- **Antecedent:** —
- **Consequent:** success → `Report` (pass-through). A disk-write/marshal failure here wraps **no** sentinel and has **no code in the frozen error taxonomy** (`report.schema.json` `x-exit-codes` has none for it) — **out of scope for this design, unchanged** (frozen contracts are not modified by this stage). It is nevertheless a *reachable* path, and it is exactly the concrete instance that exercises `cli.ResolveExitCode`'s total-function fallback → exit **3** (see that card, D3/C4). Pinned by `cmd/app/main_test.go`, **not** by a component scenario; **not** unit-tested (I/O module, Step 8.1).

## 4. Unit-test formula (Step 8.1)

`N = 1 (happy) + Σ (antecedent branches that yield a distinguishable consequent)`. Head, I/O
modules, and adapters are **not** unit-tested (`module-tree.md` §3, Step 8.1 hard rule). **This rule
sentence is unchanged and is not amended by this change** (BRD C0/Q2: no `adapter_test.go`, no head
row, no I/O row).
...
| **`BuildContractParser`** *(new row)* | 1 | none — binds an already-validated scalar, no antecedent branch | 0 | **1** |
| **`ContractParser.Parse`** *(renamed from `NewConsumedContract`)* | 1 | undecodable content; `schema_version != "1.0"`; `consumer != expectedConsumer`; `captured_hash` regex mismatch; empty `channels`; channel missing `address`/`protocol`; channel with neither `sends` nor `receives` | 7 | 8 |
...
| **`BuildReporter`** *(new row)* | 1 | none — binds the clock port, no antecedent branch | 0 | **1** |
| **`Reporter.Fold`** *(renamed from `FoldReport`)* | 1 | `compatible ⇔ errors == []` both directions | 1 | 2 |
...
Total unit tests (this slice): **51** (was 49; `49 + 1 BuildReporter + 1 BuildContractParser`).
`R1–R9` are covered **here**, one rule at a time (`TASK.md` DoD: "Юнит-тестами покрыто ядро: R1–R9
поштучно…") — not as component scenarios (§6).

> **Deliberate divergence from the BRD, flagged (design decision, not a slip).** BRD **N7/D1** pins
> this total at **49**, assuming the two affected rows are merely *renamed* in place — i.e. that a
> factory/product-method pair is **one** node. `module-tree.md` §3's own convention says **two**
> (`BuildSpecLoader`/`FileSpecLoader.Load`, `BuildReportWriter`/`ReportWriter.Write`, both already
> shipped, both split, both with the factory carrying its own §4 row at N=2), and consistency with
> the shipped pattern is the tie-breaker `change-delta.md` §5.1 names. Applying it yields 21 tree
> nodes and **51** here. **The BRD's 49 is therefore wrong** and should be read as superseded by
> ADR-001; forcing the count to 49 would either merge two independently-changeable hidden decisions
> into one module or leave two logic factories silently untested, both of which contradict rules the
> same BRD asks to keep verbatim.
>
> What the BRD gets right and this change honors exactly: **no adapter row, no head row, no I/O row,
> no carve-out of the "head/adapters are not unit-tested" rule** (C0/Q2), and the two new Go tests
> the change adds (`main_test.go` cases, the `ProcessValidate` determinism anchor) are **not**
> §4 rows — the determinism anchor carries the documented non-formula marker (BRD I1c/DEC-3).

## 5. Gherkin ↔ module reconciliation (Step 8.4)

One row per distinguishable outcome the component suite (§6) asserts. **+3 rows** vs the canonical
document (marked **new**), per BRD D2.
...
| **happy: compatible pair (new, I2)** | **stdout bytes equal `fixtures/validate/good/report.baseline.json` — full byte equality, with only `generated_at`'s *value* normalized identically on both sides (it must be present and parse as RFC3339)** | **`Reporter.Fold` (canon-1.1 shape, key order, indentation) + `Report.WriteJSON`** |
| **incompatible: exit-1 verdict (new, C3)** | **exit 1, stdout is a schema-valid canon-1.1 report, `compatible == false`** | **`CompareContracts` (violations ≠ ∅) → `Reporter.Fold` → `cli.ResolveExitCode` (grid row 2)** |
| **incompatible: exit-1 verdict (new, C3)** | **`errors[0].code == "CHANNEL_NOT_IN_PROVIDER"`** | **`ResolveChannelDirection` (R1)** |
...
### C2 — the `cli.ResolveExitCode` grid, proven 7/7 through the binary

Recorded here (BRD D2, C2) so a reviewer can check it mechanically. The binary path **is** production,
so "every error wrapped exactly as production raises it" holds by construction — strictly stronger
than a hand-built unit grid, and it is why no adapter unit test is needed (C0/Q2).
...
| 2 | `err == nil && !report.Compatible` → **1** | — | `validate.feature` **new scenario 9** (C3) — *this was the single gap* |
...
**Totality beyond the grid (D3/C4):** an error matching *no* sentinel — the reachable
`ReportWriter.Write` disk failure — also lands on `default:` → **3**. It is proven by a
`cmd/app/main_test.go` case, deliberately **not** by a scenario (see §6, anti-gaming boundary).
Coverage after this change: grid **7/7 through the binary, with zero new unit tests**.
...
6a–6i). Before this change only one of the two was asserted anywhere, so the tool's headline
behaviour ("this consumer is incompatible") was proven by nothing (BRD C3). Adding it is a
**happy-class member, not an 8th adapter branch** — the adapter-branch count is unchanged at 7.
...
Fixtures for the 8 pre-existing scenarios already exist as a dry trace in
[`../../../../sandbox/EMULATION.md`](../../../../sandbox/EMULATION.md) (S0/S11–S15, S12/S3a-equivalent,
S13/S4b-equivalent — see the table below for the exact mapping); the component-test realize stage
(`wirth-tester`) drops these into `.feature` files, not new ones.

### Scenario table (Cockburn wording verbatim; tag `@wip` on the new surface only)
...
| **9** | **`@wip` Consumer incompatible with provider (primary verdict)** | **happy-class (incompatible verdict) — NOT an adapter branch** | **NEW `fixtures/validate/incompatible/`: `good/` + one channel address added to BOTH `consumer.channels` AND the consumed-contract's `channels`, absent from the provider spec → R1** | **1** | **`CHANNEL_NOT_IN_PROVIDER`** |

**Tag discipline (BRD C3):** scenario **9 carries `@wip`**; scenarios 1–8 stay untagged (already
accepted). `@wip` is stripped by `@fagan` at slice acceptance only.

**Assertion order for scenario 9 (BRD C3, verbatim):** exit code **1**; stdout is a schema-valid
canon-`1.1` report (`report.schema.json`); `compatible == false`; `errors[0].code ==
"CHANNEL_NOT_IN_PROVIDER"`.

**Fixture trap (BRD C3, repeated because it silently proves the wrong branch):** the added address
MUST be present in the consumed-contract. Absent there, `NewComparison`'s cross-artifact check fires
→ `ErrConfigInvalid` → exit **2**, and the scenario would pass for the wrong reason. `good/`'s three
files MUST NOT be modified (I2 depends on their bytes).
...
failure (BRD C4) is **not** one of the 7 adapter branches; making it a scenario would break the gate
line above. It lives in `cmd/app/main_test.go` for exactly that reason, and it asserts **only**
`*exitError` with `code == 3` — the stdout body on that path is schema-invalid today and pinning it
exceeds the `patch` envelope (recorded as debt, BRD N1).

Downstream consequences of this section, for the realize stage: `component-tests/features/validate.feature`'s
header formula becomes the one above, and its STOP-warning shifts from "a 9th scenario" to "a
**10th** scenario"; `node harness/validate-component-tests.mjs .` must exit 0 and report **9**
business scenarios, the new surface as `@wip`. Realize stage: `wirth-tester` (component-tests skill,
REALIZE half) — this design stage writes no `.feature`.
